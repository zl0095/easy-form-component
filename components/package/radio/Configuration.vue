          ...
          <div class="easy-form-option-container">
            ...
            <div class="easy-form-option-label">
              ...
              <input v-model="item.label"
                     ...
                     :name="item.label" placeholder="请输入选项值" class="easy-form-option-item"
            ...
            <div class="easy-form-setting" :style="{ opacity : showBackground === index + 2 ? 1 : 0}">
              <SvgIcon style="margin-left: 5px;cursor: pointer" name="reduce_active" @click="()=>delOption(index)"
...
import {RadioConfigurationOptions} from '../../interface';
...
const props = defineProps({
  modelValue: {
    type: Object as PropType<{
      title: string;
      direction?: string;
      options: Array<RadioConfigurationOptions>;
      required: boolean;
      error: string
    }>,
    default: () => ({
...
function watchBlur(label: string, index: number, event: Event) {
  ...
  if (propsModelValue.value.options.filter((item: RadioConfigurationOptions, indexs: number) => indexs !== index).some((item: RadioConfigurationOptions) => item.label === label)) {
