<template>
    ...
    <div class="easy-form-content">
      ...
      <div class="easy-form-table-container"
        ...
        <table class="easy-form-table"
          ...
          <tr v-if="modelValue.columns.length">
          ...
          <tr v-for="(row, rowIndex) in modelValue.data" :key="rowIndex">
            <td v-for="(column, columnIndex) in modelValue.columns" :key="columnIndex"
              ...
              <input v-if="columnIndex === 0" v-model="row.index" class="easy-form-editable" style="border: none;"
                     type="text">
              <template v-else>
                <span class="easy-form-star-rating" v-for="item in 5" :key="item"
                     ...
                     <SvgIcon  v-if="item <= (Number(row[column.key]) || 0)" name="evaluate_active" style="width: 15px;height: 15px; animation: shake 0.2s;
  animation-iteration-count: 1;" />
              ...
              <div v-if="columnIndex===0" class="easy-form-move">
                <SvgIcon name="more" class="easy-form-more"
                         @click="showMoveTopHandler(rowIndex)" />
...
const emit = defineEmits(['blurTitle', 'focusTitle', 'inputTitle', 'update:data', 'update:columns', 'update:modelValue'])
...
function watchTitleInput(title: string, event: Event) {
  if (props.maxLength && title.length > parseInt(props.maxLength.toString(), 10)) {
    localTitle.value = props.modelValue.title.substring(0, props.maxLength as number)
  }
...
function addColumn() {
  ...
  totalColumnsWidth.value += columnWidth;
}
...
function addRow() {
  const newRow: Row = {
    index: `矩阵${props.modelValue.data.length + 1}`, fill1: 0,
    fill2: 0,
  };
  ...
  props.modelValue.columns.forEach((column) => {
    if (column.key === 'index') {
      ...
      newRow[column.key] = 0; // 这里假设每一列都有一个默认值 0
    }
  });
...
function deleteColumn(index: number) {
  // 要删除的列的key
  const deletedColumnKey = props.modelValue.columns[index]?.key;
  if (deletedColumnKey) {
    // 从 columns 数组中删除指定索引的列
    props.modelValue.columns.splice(index, 1);
    emit('update:columns', [...props.modelValue.columns]);
    props.modelValue.data.forEach(row => {
      if (deletedColumnKey in row) {
        delete row[deletedColumnKey];
      }
    });
    emit('update:data', [...props.modelValue.data]);
  }
  ...
  if (count > 0) {
    for (let i = 0; i < count; i++) {
      const newColumn = {
        ...
        key: `fill${(props.modelValue.columns?.length ?? 0)}`
      };
      props.modelValue.columns.push(newColumn);
      emit('update:columns', [...props.modelValue.columns]);
    }
