<template>
  <div id="box" class="easy-form-box">
    <div v-if="!isMobile()" class="easy-form-content" :style="{width:props.modelValue.columns.length<= 3 ? '60%' :'100%' }">
        ...
        <tr v-for="(row, rowIndex) in props.modelValue.data" :key="rowIndex">
          <td v-for="(column, columnIndex) in props.modelValue.columns" :key="columnIndex">
            ...
            <template v-else>
              <input
                  type="radio"
                  :id="`row-${rowIndex}-option-${columnIndex}`"
                  :value="columnIndex"
                  v-model="row.selected"
                  :style="{ transform: 'scale(1.5)' }"
                  :name="`rowGroup-${rowIndex}`">
            </template>
          </td>
        </tr>
      </table>
...
props.modelValue.data.forEach((row, rowIndex) => {
  if (!row.selected) {
...
input.easy-form-editable-header {
  border: none;
  width: 100%;
  box-sizing: border-box;
  height: 32px;
