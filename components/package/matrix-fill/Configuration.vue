          ...
          <tr v-for="(row, rowIndex) in modelValue.data" :key="rowIndex">
            <td v-for="(column, columnIndex) in modelValue.columns" :key="columnIndex"
              ...
              <input v-if="columnIndex === 0" v-model="row.index" class="easy-form-editable" style="border: none;"
                     type="text">
              <input v-else v-model="(row as any)[column.key]" class="easy-form-editable" type="text">
...
const containerWidth = ref(770); // 假设容器宽度为770px
...
const emit = defineEmits(['update:modelValue', 'blurTitle', 'focusTitle', 'inputTitle', 'update:data', 'update:columns', 'update:modelValue.title'])
...
function addColumn() {
  const newColumn = {
    title: `填空${(props.modelValue.columns?.length ?? 0)}`,
...
function moveColumnRight(index: number) {
  if (props.modelValue.columns !== undefined && index < props.modelValue.columns.length - 1) {
    if (index < props.modelValue.columns.length - 1) {
      const [movedColumn] = props.modelValue.columns.splice(index, 1);
      props.modelValue.columns.splice(index + 1, 0, movedColumn);
      emit('update:columns', [...props.modelValue.columns]);
      showMoveTop.value = -1
